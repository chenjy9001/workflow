$(function() {
	...
	$.ajax({
		type : 'POST',
		url : 'viewAsync.htm?flag=10',
		dateType : 'json',
		success : function(data) {
			var dataObj = eval("(" + data + ")");
			for (var i = 0; i < dataObj.data.length; i++) {
				if (dataObj.data[i].paramCode == "Logo") {
					$("#Logo").parent().parent().find(".fileinput-return").html("<img src='" + basePath + "fileserver/loadImage/" + dataObj.data[i].paramValue + "' style='max-width: 300px;max-height:90px;width: auto !important;height:auto !important;' />");
