...
(function(root, factory) {
	...
	DateRangePicker.prototype = {
		...
		move : function() {
			// start 重新计算范围控件的位置 add by tanrq
			this.container.css({
				"float" : "left"
			});
			if (this.container.find("div.left,div.right").is(":visible")) {
				this.container.css({
					"width" : this.container.find("div.ranges").outerWidth(true)+this.container.find(".calendar.left").outerWidth(true)+this.container.find(".calendar.right").outerWidth(true)+11
				});
			} else {
				this.container.css({
					"width" : "auto"
			...
			var tagHeight = this.element.outerHeight();
			var tagWidth = this.element.outerWidth();
			var tagTop = this.element.offset().top - $(window).scrollTop();
			var tagBottom = $(window).height() - tagTop - tagHeight;
			var tagLeft = this.element.offset().left - $(window).scrollLeft() + tagWidth;
			var tagRight = $(window).width() - tagLeft + tagWidth;
			var pickerHeight = this.container.outerHeight();
			var pickerWidth = this.container.outerWidth();
			// console.info("tagBottom:" + tagBottom + "\tpickerHeight:" + pickerHeight + "\ttagLeft:" + tagLeft + "\ttagRight:" + tagRight + "\tpickerWidth:" + pickerWidth);
			...
			if (tagRight > pickerWidth) {
				this.opens = 'right';
			} else if (tagLeft > pickerWidth) {
				this.opens = 'left';
			} else {
				this.opens = 'center';
			}
			this.container.removeClass('opensright').removeClass('opensleft').removeClass('openscenter');
			this.container.addClass('opens' + this.opens);
			// end 重新计算日期范围控件的位置 add by tanrq
			var parentOffset = {
					top : 0,
					left : 0
				}, containerTop;
		...
		hide : function(isUpdateElement) {
			...
			// hide()函数增加isUpdateElement参数，用于控制日期方位选择框隐藏时是否赋值给input
			// 这里主要解决该日期范围选择器，无法清空input的问题 add by tanrq
			if (isUpdateElement == undefined || isUpdateElement == true) {
				this.updateElement();
			}
