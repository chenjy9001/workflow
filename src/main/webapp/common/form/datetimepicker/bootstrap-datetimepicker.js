...
(function(factory) {
	...
	var Datetimepicker = function(element, options) {
		...
		this.clickedOutside = function(e) {
			...
			// 解决点击输入框时，日期选择器就被隐藏的bug add by tanrq
			if ($(e.target).closest('.datetimepicker').length === 0 && (that.element && e.target != that.element[0])) {
				that.hide();
			}
		}
	...
	Datetimepicker.prototype = {
		...
		show : function(e) {
			// 解决点击输入框以后，日期选择器被重置的问题 add by tanrq
			...
			// 显示日期选择器时，禁止body滚动，防止日期选择器和输入框偏移 add by tanrq
			if ($.fn.mousewheel) {
		...
		place : function() {
			...
			// start 重新计算日期控件的位置 add by tanrq
			var tagHeight;
			if (this.component) {
				tagHeight = this.component.outerHeight();
			} else {
				tagHeight = this.element.outerHeight();
			}
			var tagTop = offset.top - $(window).scrollTop();
			var tagBottom = $(window).height() - tagTop - tagHeight;
			var pickerHeight = this.picker.outerHeight();
			if (tagBottom > pickerHeight) {
				this.pickerPosition = "bottom-right";
				this.picker.attr('class', 'datetimepicker datetimepicker-dropdown-bottom-right dropdown-menu');
			} else if (tagTop > pickerHeight) {
				this.pickerPosition = "top-right";
				this.picker.attr('class', 'datetimepicker datetimepicker-dropdown-top-right dropdown-menu');
			}
			// end 重新计算日期控件的位置 add by tanrq
		...
		fill : function() {
			...
			var d = new Date(this.viewDate), year = d.getUTCFullYear(), month = d.getUTCMonth(), dayMonth = d.getUTCDate(), hours = d.getUTCHours(), minutes = d.getUTCMinutes(), startYear = this.startDate !== -Infinity ? this.startDate.getUTCFullYear() : -Infinity,
			/* 修复年视图，设置起始时间，显示不对的bug。 modify by tanrq */
			/* startMonth = this.startDate !== -Infinity ? this.startDate.getUTCMonth() : -Infinity, */
			startMonth = this.startDate !== -Infinity ? this.startDate.getUTCMonth() + 2 : -Infinity, endYear = this.endDate !== Infinity ? this.endDate.getUTCFullYear() : Infinity,
			/* 修复年视图，设置结束时间，显示不对的bug。 modify by tanrq */
			/* endMonth = this.endDate !== Infinity ? this.endDate.getUTCMonth() + 1 : Infinity, */
			endMonth = this.endDate !== Infinity ? this.endDate.getUTCMonth() + 3 : Infinity
			currentDate = (new UTCDate(this.date.getUTCFullYear(), this.date.getUTCMonth(), this.date.getUTCDate())).valueOf(), today = new Date();
			...
			this.fillMonths();
			/*
			 * var prevMonth = UTCDate(year, month, 0,0,0,0,0); prevMonth.setUTCDate(prevMonth.getDate() - (prevMonth.getUTCDay() - this.weekStart + 7)%7);
			 */
		...
		showMode : function(dir) {
			...
			/*
			 * vitalets: fixing bug of very special conditions: jquery 1.7.1 + webkit + show inline datetimepicker in bootstrap popover. Method show() does not set display css correctly and datetimepicker is not shown. Changed to .css('display', 'block') solve the problem. See https://github.com/vitalets/x-editable/issues/37
			 * 
