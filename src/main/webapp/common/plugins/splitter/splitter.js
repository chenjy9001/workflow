...
(function($) {
	...
	$.fn.splitter = function(args) {
		...
		return this.each(function() {
			...
			var opts = $.extend({
				minAsize : 0, // minimum width/height in PX of the first (A)
								// div.
				maxAsize : 0, // maximum width/height in PX of the first (A)
								// div.
				minBsize : 0, // minimum width/height in PX of the second (B)
								// div.
				maxBsize : 0, // maximum width/height in PX of the second (B)
								// div.
				ghostClass : 'working',// class name for _ghosted splitter and
										// hovered button
			...
			var slave = args.slave;// optional, elemt forced to receive resize
									// event
			// Create splitbar
			...
			function startDrag(e) {
				...
				_ghost.addClass(opts.ghostClass).css('position', 'absolute').css('z-index', '250').css("-webkit-user-select", "none").width(C.width()).height(C.height()).css(opts.moving,
			...
			function endDrag(e) {
				...
				_ghost.remove();
				_ghost = null;
				mychilds.css("-webkit-user-select", "text");// let Safari select
															// text again
			...
			function splitTo(perc, reversedorder, fast) {
				...
				_ismovingNow = true;
				if (splitPos && splitPos > 10 && splitPos < 90)// do not save
																// accidental
																// events
					_splitPos = splitPos;
				splitPos = perc;

				var barsize = C[opts.sizing]() + (2 * parseInt(C.css('border-' + opts.moving + '-width')));// +
																											// border.
																											// cehap&dirty
				...
				if (reversedorder) {// reduces flickering if total percentage
									// becomes more than 100 (possible while
									// animating)
					var anob = {};
